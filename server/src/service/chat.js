...
async function writeAndSend(type, roomKey, writeMsg, sendMsg) {
  ...
  if (type === "group") {
    ...
    for (const key of userIdWraps) {
      if (userIdWraps[key].user_id !== sendMsg.senderId) {
        pub({ receiverId: userIdWraps[key].user_id, type: "wsFetchMsgList" });
      }
    }
  } else {
    pub({ receiverId: sendMsg.receiverId, type: "wsFetchMsgList" });
  }
}
 ...
 * @description get
 */
export async function findAllMsgLists(req, res) {
  try {
    /**
...
limit 1;
      `;
      const result4 = await query(sql4, [item.room_key]);
      item["latest_msg"] = result4[0].latest_msg;
      item["media_type"] = result4[0].media_type;
      // item["avatar"] = "";
    }
    if (results3) {
    ...
    console.log("findMsgList data:", data);
