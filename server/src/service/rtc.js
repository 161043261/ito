 ...
 * @description ws: roomKey, email, type
 */
export async function createRtc(ws, req) {
  ...
  try {
    ...
    ws.on("message", async (msgStr) => {
       ...
       * @description { cmd, iceCandidate, mode, data, receiverList }
       */
      ...
      switch (msgObj.cmd) {
        case RtcCmd.CreateRtcRoom:
          ...
          if (!global.OnlineUsers[email].state) {
            ws.send(JSON.stringify({ code: BaseState.Err, msg: "您正在音视频通话" }));
            return;
          }
          if (type === "friend") {
            ...
            if (global.OnlineUsers[receiverList[0].email].state) {
              ws.send(JSON.stringify({ code: BaseState.Err, msg: "对方正在音视频通话" }));
            }
          } else {
            // type === 'group'
 ...
 * @description get: roomKey
 * @description 获取当前房间中, 正在聊天的所有用户的邮箱
 */
